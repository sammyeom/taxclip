...
export default function Home() {
  ...
  const [appComingSoonModal, setAppComingSoonModal] = useState(false);
  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      <section className="py-16 sm:py-24 bg-gradient-to-br from-slate-900 to-slate-800">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <motion.div
            initial={{ opacity: 0, y: 20 }}
            whileInView={{ opacity: 1, y: 0 }}
          ...
          >
            ...
            <p className="text-lg sm:text-xl text-slate-400 mb-10 max-w-2xl mx-auto">
              Scan receipts on the go. Available soon on iOS and Android.
            </p>

            <div className="flex flex-col sm:flex-row gap-4 justify-center">
              <motion.button
                onClick={() => setAppComingSoonModal(true)}
                whileHover={{ scale: 1.05 }}
                whileTap={{ scale: 0.95 }}
                className="flex items-center justify-center gap-3 bg-white text-slate-900 px-8 py-4 rounded-xl font-semibold shadow-xl hover:bg-slate-100 transition-colors"
              >
      ...
      {appComingSoonModal && (
        <motion.div
          ...
          animate={{ opacity: 1 }}
          exit={{ opacity: 0 }}
          onClick={() => setAppComingSoonModal(false)}
          className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4"
        >
          <motion.div
            initial={{ scale: 0.9, opacity: 0 }}
            ...
            transition={{ type: "spring", duration: 0.5 }}
            onClick={(e) => e.stopPropagation()}
            className="bg-white rounded-2xl shadow-2xl max-w-md w-full relative overflow-hidden p-8 text-center"
          >
            {/* Close button */}
            <button
              onClick={() => setAppComingSoonModal(false)}
            ...
            <p className="text-slate-600 mb-6">
              The TaxClip mobile app is currently in development and will be available soon on the App Store and Google Play.
            </p>
            ...
            <div className="bg-slate-100 rounded-xl p-4 mb-6">
              ...
              <p className="text-sm text-slate-500 mt-1">
                Sign up for TaxClip and we&apos;ll let you know when the app launches!
              </p>
            </div>

            <button
              onClick={() => {
                setAppComingSoonModal(false);
                router.push('/sign-up');
              }}
              className="w-full bg-cyan-500 hover:bg-cyan-600 text-white font-semibold py-3 px-6 rounded-xl transition-colors"
            >
              Sign Up for Updates
            </button>
          </motion.div>
