...
export default function SplitView({
  ...
  // Helper to check if a field has low confidence
  const isLowConfidence = (field: 'date' | 'vendor' | 'total' | 'items') => {
    return extractedData.fieldConfidence && extractedData.fieldConfidence[field] < 70;
  };

  // Check if any field has low confidence
  const hasLowConfidenceFields = extractedData.fieldConfidence && (
    isLowConfidence('date') || isLowConfidence('vendor') || isLowConfidence('total') || isLowConfidence('items')
  );
        ...
        <div className="bg-white rounded-xl border border-gray-200 overflow-hidden flex flex-col">
          ...
          <div className={`flex-1 overflow-auto p-4 ${isFullscreen ? 'h-[calc(100vh-120px)]' : ''}`}>
            ...
            {hasLowConfidenceFields && (
              <div className="mb-4 p-3 bg-amber-50 border border-amber-200 rounded-lg flex items-center gap-3">
                ...
                <p className="text-sm text-amber-700">
                  Some fields may need review. Look for highlighted fields below.
                </p>
              </div>
            )}
            ...
            <div className="grid grid-cols-2 gap-3 mb-6">
              {/* Vendor */}
              <div className={`col-span-2 bg-gradient-to-r from-cyan-50 to-blue-50 rounded-lg p-4 border ${isLowConfidence('vendor') ? 'border-amber-400 border-2' : 'border-cyan-100'}`}>
              ...
              {/* Amount */}
              <div className={`bg-gradient-to-r from-green-50 to-emerald-50 rounded-lg p-4 border ${isLowConfidence('total') ? 'border-amber-400 border-2' : 'border-green-100'}`}>
              ...
              {/* Date */}
              <div className={`bg-gradient-to-r from-purple-50 to-indigo-50 rounded-lg p-4 border ${isLowConfidence('date') ? 'border-amber-400 border-2' : 'border-purple-100'}`}>
            ...
            {(() => {
              ...
              return (
                <div className={`border rounded-lg overflow-hidden ${isLowConfidence('items') ? 'border-amber-400 border-2' : 'border-gray-200'}`}>
                  <div className="bg-gray-50 px-4 py-3 border-b border-gray-200 flex items-center justify-between">
                    <h3 className="text-sm font-semibold text-gray-700 flex items-center gap-2">
                      <List className="w-4 h-4" />
                      Line Items ({normalizedItems.length})
                      {isLowConfidence('items') && (
                        <span className="inline-flex items-center gap-1 px-1.5 py-0.5 bg-amber-100 text-amber-700 rounded text-xs font-medium">
                          <AlertTriangle className="w-3 h-3" />
                          Check
                        </span>
