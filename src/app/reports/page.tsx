...
export default function ReportsPage() {
  ...
  const [taxSummaryYear, setTaxSummaryYear] = useState(currentYear);
  ...
  // Tax summary calculations based on selected year
  const taxSummary = useMemo(() => {
    // Filter receipts by selected tax summary year
    const yearReceipts = allReceipts.filter((r) => {
      const receiptYear = new Date(r.date).getFullYear();
      return receiptYear === taxSummaryYear;
    });

    if (yearReceipts.length === 0) {
      return { totalExpenses: 0, deductibleAmount: 0, estimatedSavings: 0, receiptCount: 0 };
    }

    const TAX_SAVINGS_RATE = 0.373; // 22% federal + 15.3% self-employment
    const totalExpenses = yearReceipts.reduce((sum, r) => sum + getReceiptTotal(r), 0);
    const mealsAmount = yearReceipts
          ...
          <>
            ...
            <Card className="mb-6 sm:mb-8 bg-gradient-to-r from-emerald-50 to-cyan-50 border-emerald-200">
              <CardHeader className="pb-2 sm:pb-4">
                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
                  ...
                  <DropdownMenu>
                    <DropdownMenuTrigger asChild>
                      <Button variant="outline" size="sm" className="flex items-center gap-2 h-8 px-3 bg-white border-emerald-200 hover:border-emerald-400 hover:bg-emerald-50">
                        <Calendar className="w-3.5 h-3.5 text-emerald-600" />
                        <span className="font-semibold text-emerald-700">{taxSummaryYear}</span>
                    ...
                    <DropdownMenuContent align="end" className="w-48">
                      {[currentYear, currentYear - 1, currentYear - 2].map((year) => (
                        <DropdownMenuItem
                          key={year}
                          onClick={() => setTaxSummaryYear(year)}
                          className={`flex items-center gap-3 py-2.5 cursor-pointer ${
                            taxSummaryYear === year ? 'bg-emerald-50' : ''
                          }`}
                        >
                          <div className={`w-4 h-4 rounded-full border-2 flex items-center justify-center ${
                            ...
                            {taxSummaryYear === year && <Check className="w-2.5 h-2.5 text-white" />}
                          </div>
                          <div className="flex-1">
                            <div className="font-medium text-slate-900">{year}</div>
                            <div className="text-xs text-slate-500">
                              {year === currentYear ? 'Current year' : year === currentYear - 1 ? 'Last year' : `${year}`}
                            </div>
                          </div>
                        </DropdownMenuItem>
                      ))}
                ...
                {taxSummary.receiptCount > 0 && (
                  <p className="text-xs text-slate-500 mt-1">
                    Based on {taxSummary.receiptCount} receipt{taxSummary.receiptCount !== 1 ? 's' : ''}
                  </p>
                )}
              </CardHeader>
              <CardContent>
